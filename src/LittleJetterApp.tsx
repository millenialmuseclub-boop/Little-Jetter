...
// to check. Updates are served as signed static files from Cloudflare R2.
...
const destinations: Destination[] = [
  ...
  { id: 'kyoto', region: 'Asia', city: 'Kyoto', country: 'Japan', area: 'Kyoto Prefecture', areaType: 'Prefecture', note: 'Cherry blossoms & quiet temples', weather: 'Mild + petal-breezy', prompt: 'A layer you can tie around your waist is a smart travel trick.', color: '#e592ab', icon: '❀', adventure: 'You’re walking beneath pink blossom trees, ringing a temple bell, and spotting a cat napping on a warm stone step.', exploreIcon: '🌸', notices: [{icon:'🌸',label:'A petal landing on your sleeve'},{icon:'⛩️',label:'A bright orange gate'},{icon:'🐈',label:'A cat napping in the sun'},{icon:'🍡',label:'A stack of sweet dumplings'}], needsLayer: true, sandalsFriendly: false, passportPhrase: 'Blossom-path wanderer' },
  { id: 'bangkok', region: 'Asia', city: 'Bangkok', country: 'Thailand', area: 'Bangkok', areaType: 'Region', note: 'River boats & golden temples', weather: 'Hot + humid', prompt: 'Choose something light and breathable for a steamy day.', color: '#d9a441', icon: '☼', adventure: 'You’re riding a long-tail boat down the river, spotting a golden spire, and trying a new fruit at a floating market.', exploreIcon: '🛶', notices: [{icon:'🛶',label:'A long-tail boat gliding by'},{icon:'🛕',label:'A golden temple spire'},{icon:'🥭',label:'A fruit you have never tried'},{icon:'🎐',label:'Wind chimes near a doorway'}], needsLayer: false, sandalsFriendly: true, passportPhrase: 'River-market explorer' },
  { id: 'santorini', region: 'Europe', city: 'Santorini', country: 'Greece', area: 'South Aegean', areaType: 'Region', note: 'White walls & bright blue domes', weather: 'Sunny + breezy', prompt: 'A sun hat and easy sandals are perfect here.', color: '#3d7dbf', icon: '◐', adventure: 'You’re climbing whitewashed stairs, spotting a blue-domed church, and watching sailboats cross the caldera.', exploreIcon: '⛵', notices: [{icon:'⛪',label:'A perfectly blue dome'},{icon:'🐚',label:'A shell on a sunny step'},{icon:'⛵',label:'A sailboat far below'},{icon:'🍋',label:'A lemon tree in a courtyard'}], needsLayer: false, sandalsFriendly: true, passportPhrase: 'Whitewashed-stairway climber' },
  { id: 'dubai', region: 'Asia', city: 'Dubai', country: 'United Arab Emirates', area: 'Dubai', areaType: 'Region', note: 'Towers, dunes & desert sunsets', weather: 'Hot + sunny', prompt: 'Light layers and sun protection make the best travel gear.', color: '#c99b56', icon: '◇', adventure: 'You’re riding to the top of a shining tower, feeling warm sand between your toes, and watching the sky turn gold at sunset.', exploreIcon: '🏙️', notices: [{icon:'🏙️',label:'A tower reaching past the clouds'},{icon:'🐫',label:'A camel resting in the shade'},{icon:'🏜️',label:'A dune shaped by the wind'},{icon:'🌇',label:'A golden desert sunset'}], needsLayer: false, sandalsFriendly: true, passportPhrase: 'Skyline-and-sand explorer' },
  { id: 'istanbul', region: 'Europe', city: 'Istanbul', country: 'Türkiye', area: 'Istanbul', areaType: 'Region', note: 'Bazaars & domes on two continents', weather: 'Mild + breezy', prompt: 'A layer for the ferry breeze is a clever packing choice.', color: '#b4534a', icon: '☾', adventure: 'You’re crossing between two continents by ferry, wandering a colorful bazaar, and counting minarets against the sky.', exploreIcon: '⛴️', notices: [{icon:'⛴️',label:'A ferry crossing the strait'},{icon:'🏮',label:'A lantern in the bazaar'},{icon:'🐈',label:'A cat sunbathing on a mosque step'},{icon:'🍮',label:'A wobbly sweet treat'}], needsLayer: true, sandalsFriendly: false, passportPhrase: 'Two-continent voyager' },
  { id: 'lima', region: 'South America', city: 'Lima', country: 'Peru', area: 'Lima', areaType: 'Region', note: 'Ocean cliffs & colorful murals', weather: 'Mild + misty', prompt: 'A light layer helps with the coastal mist.', color: '#5a8fa6', icon: '≈', adventure: 'You’re walking along a clifftop path above the ocean, discovering a mural bursting with color, and tasting a citrusy seaside snack.', exploreIcon: '🌊', notices: [{icon:'🌊',label:'Waves meeting the cliffs'},{icon:'🎨',label:'A mural full of color'},{icon:'🦅',label:'A bird gliding on the wind'},{icon:'🍋',label:'A citrusy seaside snack'}], needsLayer: true, sandalsFriendly: false, passportPhrase: 'Clifftop mural spotter' },
  { id: 'montreal', region: 'North America', city: 'Montreal', country: 'Canada', area: 'Quebec', areaType: 'Province', note: 'Cobblestones & maple sweetness', weather: 'Crisp + colorful', prompt: 'A cozy layer belongs on this cobblestone stroll.', color: '#a35a3d', icon: '❧', adventure: 'You’re strolling cobblestone lanes in the old port, hearing music drift from a café, and trying a warm maple treat.', exploreIcon: '🍁', notices: [{icon:'🍁',label:'A leaf turning bright red'},{icon:'🎷',label:'Music drifting from a café'},{icon:'🕰️',label:'An old stone clocktower'},{icon:'🥞',label:'A warm maple treat'}], needsLayer: true, sandalsFriendly: false, passportPhrase: 'Cobblestone-lane stroller' },
  { id: 'queenstown', region: 'Oceania', city: 'Queenstown', country: 'New Zealand', area: 'Otago', areaType: 'Region', note: 'Mountains & lakeside adventure', weather: 'Cool + crisp', prompt: 'A warm layer is ready for mountain air.', color: '#3c7a63', icon: '▲', adventure: 'You’re skipping stones on a mountain lake, spotting snowy peaks, and warming up with a treat after a day outdoors.', exploreIcon: '🏔️', notices: [{icon:'🏔️',label:'A snow-capped peak'},{icon:'🚡',label:'A gondola climbing the hill'},{icon:'🦆',label:'A duck paddling the lake'},{icon:'🍫',label:'A chocolatey treat at the end of the day'}], needsLayer: true, sandalsFriendly: false, passportPhrase: 'Mountain-lake adventurer' },
  { id: 'petra', region: 'Asia', city: 'Petra', country: 'Jordan', area: 'Ma’an', areaType: 'Region', note: 'A rose-red city carved in stone', weather: 'Warm + dry', prompt: 'Sturdy shoes and sun protection make this trek comfortable.', color: '#c1795a', icon: '⛰', adventure: 'You’re walking through a narrow stone canyon, discovering a carved treasury glowing rose-pink, and spotting a lizard sunning on a rock.', exploreIcon: '🏜️', notices: [{icon:'🏛️',label:'A carved facade glowing pink'},{icon:'🦎',label:'A lizard warming on a rock'},{icon:'🐴',label:'A horse-drawn cart passing by'},{icon:'🏜️',label:'A canyon wall striped like sunset'}], needsLayer: false, sandalsFriendly: false, passportPhrase: 'Rose-stone canyon explorer' },
  { id: 'maldives', region: 'Asia', city: 'Maldives', country: 'Maldives', area: 'Kaafu', areaType: 'Region', note: 'Turquoise water & wooden walkways', weather: 'Warm + sunny', prompt: 'Swimwear and bare feet are the whole packing list here.', color: '#2ea3a3', icon: '〰', adventure: 'You’re walking a wooden path over turquoise water, spotting a fish through the clear waves, and watching the sunset from a hammock.', exploreIcon: '🐠', notices: [{icon:'🐠',label:'A fish flashing under clear water'},{icon:'🌴',label:'A palm leaning over the sand'},{icon:'🛶',label:'A boat resting on the shore'},{icon:'🌅',label:'A sunset over calm water'}], needsLayer: false, sandalsFriendly: true, passportPhrase: 'Turquoise-water wanderer' },
  { id: 'machu-picchu', region: 'South America', city: 'Machu Picchu', country: 'Peru', area: 'Cusco', areaType: 'Region', note: 'Ancient stone terraces in the clouds', weather: 'Cool + misty', prompt: 'A layer that packs small is perfect for changing mountain weather.', color: '#4f7a52', icon: '⛰', adventure: 'You’re climbing stone terraces above the clouds, spotting a llama grazing nearby, and imagining the city as it once was.', exploreIcon: '🦙', notices: [{icon:'🦙',label:'A llama grazing on a terrace'},{icon:'☁️',label:'Clouds drifting below the peak'},{icon:'🧱',label:'Stones fitted perfectly together'},{icon:'🌄',label:'The sun rising over the mountains'}], needsLayer: true, sandalsFriendly: false, passportPhrase: 'Cloud-terrace climber' },
  { id: 'zanzibar', region: 'Africa', city: 'Zanzibar', country: 'Tanzania', area: 'Zanzibar', areaType: 'Region', note: 'Spice markets & white-sand beaches', weather: 'Warm + breezy', prompt: 'Light, easy layers are perfect for spice-market days and beach afternoons.', color: '#3f9088', icon: '❉', adventure: 'You’re wandering a spice market full of new smells, spotting a dhow boat with a triangle sail, and cooling off on a white-sand beach.', exploreIcon: '⛵', notices: [{icon:'🌶️',label:'A basket of colorful spices'},{icon:'⛵',label:'A dhow with a triangle sail'},{icon:'🐚',label:'A shell on white sand'},{icon:'🚪',label:'A carved wooden door'}], needsLayer: false, sandalsFriendly: true, passportPhrase: 'Spice-market beachcomber' },
  { id: 'vilnius', region: 'Europe', city: 'Vilnius', country: 'Lithuania', area: 'Vilnius County', areaType: 'County', note: 'Cobbled lanes & amber-colored charm', weather: 'Cool + crisp', prompt: 'A cozy layer suits these cobblestone streets.', color: '#a37b3f', icon: '✦', adventure: 'You’re wandering cobbled lanes past pastel buildings, discovering a hidden courtyard, and spotting amber jewelry in a shop window.', exploreIcon: '🏘️', notices: [{icon:'🏘️',label:'A row of pastel buildings'},{icon:'🔔',label:'A bell tower chiming'},{icon:'🟠',label:'A piece of amber in a window'},{icon:'🕯️',label:'A candlelit café corner'}], needsLayer: true, sandalsFriendly: false, passportPhrase: 'Cobbled-lane wanderer' },
  { id: 'amsterdam', region: 'Europe', city: 'Amsterdam', country: 'Netherlands', area: 'North Holland', areaType: 'Province', note: 'Canals, bicycles & tulip color', weather: 'Cool + breezy', prompt: 'A layer belongs on this bicycle-and-canal adventure.', color: '#d68a3c', icon: '❁', adventure: 'You’re riding along a canal path, ringing a bicycle bell, and spotting a houseboat decorated with flowers.', exploreIcon: '🚲', notices: [{icon:'🚲',label:'A bicycle with a flower basket'},{icon:'🛶',label:'A houseboat on the canal'},{icon:'🌷',label:'A row of colorful tulips'},{icon:'🧀',label:'A wheel of cheese in a shop window'}], needsLayer: true, sandalsFriendly: false, passportPhrase: 'Canal-path cyclist' },
  { id: 'reykjavik', region: 'Europe', city: 'Reykjavik', country: 'Iceland', area: 'Capital Region', areaType: 'Region', note: 'Volcanic hills & northern light', weather: 'Cold + windy', prompt: 'A warm layer and sturdy shoes are essential here.', color: '#4d6e8a', icon: '❄', adventure: 'You’re exploring a coastline of black sand, spotting steam rising from the ground, and watching the sky shimmer with color.', exploreIcon: '🌋', notices: [{icon:'🌋',label:'Steam rising from the earth'},{icon:'🐎',label:'A fluffy pony in a field'},{icon:'🏔️',label:'A snow-capped mountain'},{icon:'✨',label:'A shimmer of color in the sky'}], needsLayer: true, sandalsFriendly: false, passportPhrase: 'Northern-light seeker' },
  { id: 'marrakech', region: 'Africa', city: 'Marrakech', country: 'Morocco', area: 'Marrakech-Safi', areaType: 'Region', note: 'Lantern souks & desert-warm colors', weather: 'Warm + dry', prompt: 'A light layer helps once the desert sun dips low.', color: '#c2632e', icon: '✺', adventure: 'You’re wandering a lantern-lit souk, spotting a tower rising over the rooftops, and following the scent of warm spices.', exploreIcon: '🏮', notices: [{icon:'🏮',label:'A row of glowing lanterns'},{icon:'🕌',label:'A tower rising over the rooftops'},{icon:'🐪',label:'A camel resting in the shade'},{icon:'🍊',label:'A basket of oranges in the sun'}], needsLayer: true, sandalsFriendly: true, passportPhrase: 'Lantern-souk wanderer' },
  { id: 'singapore', region: 'Asia', city: 'Singapore', country: 'Singapore', area: 'Singapore', areaType: 'Country', note: 'Garden towers & rainforest color', weather: 'Warm + humid', prompt: 'Light, breathable layers are perfect for this garden city.', color: '#2f9e6e', icon: '❊', adventure: 'You’re wandering beneath giant tree-shaped towers, spotting a real rainforest waterfall indoors, and tasting a new fruity treat.', exploreIcon: '🌳', notices: [{icon:'🌳',label:'A tower shaped like a giant tree'},{icon:'💧',label:'An indoor waterfall'},{icon:'🦋',label:'A butterfly in the garden dome'},{icon:'🍧',label:'A colorful shaved-ice treat'}], needsLayer: false, sandalsFriendly: true, passportPhrase: 'Garden-tower explorer' },
  { id: 'lisbon', region: 'Europe', city: 'Lisbon', country: 'Portugal', area: 'Lisbon', areaType: 'Region', note: 'Tiled walls & hillside trams', weather: 'Mild + sunny', prompt: 'Comfortable shoes are a must for these hilly streets.', color: '#dd8f4a', icon: '◧', adventure: 'You’re riding a bright yellow tram up a hill, spotting patterned tiles on every wall, and watching boats cross the wide river.', exploreIcon: '🚋', notices: [{icon:'🚋',label:'A yellow tram climbing the hill'},{icon:'🔷',label:'A wall covered in painted tiles'},{icon:'⛵',label:'A boat crossing the river'},{icon:'🥮',label:'A warm custard tart'}], needsLayer: true, sandalsFriendly: true, passportPhrase: 'Tiled-hillside rider' },
  { id: 'buenos-aires', region: 'South America', city: 'Buenos Aires', country: 'Argentina', area: 'Buenos Aires', areaType: 'Province', note: 'Tango streets & jacaranda color', weather: 'Mild + breezy', prompt: 'A light layer is perfect for a café-and-plaza stroll.', color: '#8d5aa0', icon: '❋', adventure: 'You’re watching a tango dance in a plaza, spotting purple jacaranda blossoms overhead, and sharing a sweet pastry at a café.', exploreIcon: '💃', notices: [{icon:'💃',label:'A tango dance in the plaza'},{icon:'💜',label:'Purple blossoms overhead'},{icon:'📖',label:'A cozy corner bookshop'},{icon:'🥐',label:'A sweet pastry at a café table'}], needsLayer: true, sandalsFriendly: false, passportPhrase: 'Plaza-and-pastry explorer' },
  { id: 'quebec-city', region: 'North America', city: 'Quebec City', country: 'Canada', area: 'Quebec', areaType: 'Province', note: 'Stone walls & storybook streets', weather: 'Cool + crisp', prompt: 'A cozy layer suits this storybook stone city.', color: '#6b7f9b', icon: '❆', adventure: 'You’re wandering inside old stone walls, spotting a castle-like hotel on the hill, and warming up with a sweet treat.', exploreIcon: '🏰', notices: [{icon:'🏰',label:'A castle-like hotel on the hill'},{icon:'🧱',label:'A stone wall from long ago'},{icon:'🎻',label:'A street musician playing a tune'},{icon:'🥧',label:'A warm sweet treat'}], needsLayer: true, sandalsFriendly: false, passportPhrase: 'Stone-wall storybook wanderer' },
  { id: 'bali', region: 'Asia', city: 'Bali', country: 'Indonesia', area: 'Bali', areaType: 'Province', note: 'Rice terraces & temple gates', weather: 'Warm + humid', prompt: 'Light layers and easy sandals suit these terraced hills.', color: '#4c8a5a', icon: '❃', adventure: 'You’re walking between bright green rice terraces, passing through a carved stone gate, and spotting a monkey in the trees.', exploreIcon: '🌾', notices: [{icon:'🌾',label:'A terrace of bright green rice'},{icon:'⛩️',label:'A carved stone temple gate'},{icon:'🐒',label:'A monkey swinging through the trees'},{icon:'🌺',label:'A flower offering by a doorway'}], needsLayer: false, sandalsFriendly: true, passportPhrase: 'Rice-terrace wanderer' },
  { id: 'auckland', region: 'Oceania', city: 'Auckland', country: 'New Zealand', area: 'Auckland', areaType: 'Region', note: 'Harbor sails & volcanic hills', weather: 'Mild + breezy', prompt: 'A light layer is ready for a harbor breeze.', color: '#3a8fa0', icon: '≋', adventure: 'You’re watching sailboats fill the harbor, climbing a grassy volcanic hill, and spotting a native bird in the park.', exploreIcon: '⛵', notices: [{icon:'⛵',label:'Sailboats filling the harbor'},{icon:'🌋',label:'A grassy volcanic hill'},{icon:'🐦',label:'A native bird in the park'},{icon:'🌉',label:'A bridge stretching across the water'}], needsLayer: true, sandalsFriendly: true, passportPhrase: 'Harbor-hill sailor' },
  { id: 'rio-de-janeiro', region: 'South America', city: 'Rio de Janeiro', country: 'Brazil', area: 'Rio de Janeiro', areaType: 'State', note: 'Mountain views & mosaic beaches', weather: 'Warm + sunny', prompt: 'Swimwear and easy sandals are perfect for beach and mountain views.', color: '#2e9b7a', icon: '☀', adventure: 'You’re riding up a mountain for a giant statue view, walking a wave-patterned beach sidewalk, and cooling off with a fruity treat.', exploreIcon: '🏖️', notices: [{icon:'⛰️',label:'A mountain shaped like a sugarloaf'},{icon:'🌊',label:'A wave-patterned sidewalk'},{icon:'⚽',label:'A soccer game on the sand'},{icon:'🍹',label:'A cool fruity treat'}], needsLayer: false, sandalsFriendly: true, passportPhrase: 'Carioca beach-and-mountain explorer' },
  { id: 'accra', region: 'Africa', city: 'Accra', country: 'Ghana', area: 'Greater Accra', areaType: 'Region', note: 'Ocean breeze & brilliant pattern', weather: 'Warm + breezy', prompt: 'Choose light clothes, sun protection, and sandals for a bright coastal day.', color: '#d8872f', icon: '✺', adventure: 'You’re following colorful patterns through a lively market, hearing drums in the distance, and ending the day beside the ocean.', exploreIcon: '🌊', notices: [{icon:'🧺',label:'A basket woven in bright colors'},{icon:'🥁',label:'A rhythm floating through the street'},{icon:'🌴',label:'A palm moving in the sea breeze'},{icon:'🍍',label:'Fresh fruit cut for a snack'}], needsLayer: false, sandalsFriendly: true, passportPhrase: 'Pattern-spotting coast explorer' },
  { id: 'mumbai', region: 'Asia', city: 'Mumbai', country: 'India', area: 'Maharashtra', areaType: 'State', note: 'Flower markets & sea promenades', weather: 'Warm + rain possible', prompt: 'Breathable clothes and a light rain layer make a clever team.', color: '#ce6749', icon: '❋', adventure: 'You’re spotting flower garlands at a market, watching boats near a giant stone arch, and listening to raindrops tap the promenade.', exploreIcon: '🌼', notices: [{icon:'🌼',label:'A garland made from marigolds'},{icon:'🚕',label:'A bright taxi passing by'},{icon:'🌊',label:'Boats bobbing near the harbor'},{icon:'🥭',label:'A sweet mango treat'}], needsLayer: true, sandalsFriendly: true, passportPhrase: 'Monsoon-ready flower finder' },
  { id: 'copenhagen', region: 'Europe', city: 'Copenhagen', country: 'Denmark', area: 'Capital Region', areaType: 'Region', note: 'Canal colors & bicycle bells', weather: 'Cool + breezy', prompt: 'A cozy layer and comfortable shoes belong on this canal adventure.', color: '#4f86a6', icon: '◫', adventure: 'You’re riding beside colorful canal houses, counting bicycles, and finding a tiny sailboat reflected in the water.', exploreIcon: '🚲', notices: [{icon:'🚲',label:'A bicycle with a flower basket'},{icon:'⛵',label:'A sailboat on the canal'},{icon:'🏠',label:'A row of candy-colored houses'},{icon:'🥐',label:'A warm bakery treat'}], needsLayer: true, sandalsFriendly: false, passportPhrase: 'Canal-side bicycle bell ringer' },
  { id: 'salvador', region: 'South America', city: 'Salvador', country: 'Brazil', area: 'Bahia', areaType: 'State', note: 'Ribbon color & ocean rhythm', weather: 'Warm + sunny', prompt: 'Light clothes, easy shoes, and a sun hat are perfect for bright stone streets.', color: '#2b8ea1', icon: '❉', adventure: 'You’re climbing colorful streets toward the sea, tying a wish ribbon, and following music through a sunny square.', exploreIcon: '🎶', notices: [{icon:'🎀',label:'A ribbon fluttering in the breeze'},{icon:'🥁',label:'A drumbeat in the square'},{icon:'⛵',label:'A white sail beyond the rooftops'},{icon:'🥥',label:'A cool coconut treat'}], needsLayer: false, sandalsFriendly: true, passportPhrase: 'Ribbon-bright rhythm explorer' },
  { id: 'new-orleans', region: 'North America', city: 'New Orleans', country: 'United States', area: 'Louisiana', areaType: 'State', note: 'Balcony gardens & brass-band streets', weather: 'Warm + humid', prompt: 'Choose something breezy with comfortable shoes for a music-filled walk.', color: '#7c5a91', icon: '♬', adventure: 'You’re following a streetcar past leafy balconies, hearing a brass band around the corner, and sharing a powdered-sugar treat.', exploreIcon: '🎺', notices: [{icon:'🎺',label:'A trumpet shining in the sun'},{icon:'🚋',label:'A green streetcar rolling by'},{icon:'🌸',label:'Magnolias beside a balcony'},{icon:'🍩',label:'A powdered-sugar beignet'}], needsLayer: false, sandalsFriendly: true, passportPhrase: 'Balcony-and-brass-band wanderer' },
];
...
const DESTINATIONS_WITH_BACKDROP = new Set([
  ...
  'kyoto', 'bangkok', 'santorini', 'dubai', 'istanbul', 'lima', 'montreal', 'queenstown', 'petra',
  'maldives', 'machu-picchu', 'zanzibar', 'vilnius', 'amsterdam', 'reykjavik', 'marrakech',
  'singapore', 'lisbon', 'buenos-aires', 'quebec-city', 'bali', 'auckland', 'rio-de-janeiro',
  'accra', 'mumbai', 'copenhagen', 'salvador', 'new-orleans',
]);
...
const destinationTypes: Record<string, string> = {
  ...
  kyoto: 'History + city', bangkok: 'Big city', santorini: 'Beach + island', dubai: 'Big city', istanbul: 'History + city',
  lima: 'Coastal city', montreal: 'Big city', queenstown: 'Mountain + nature', petra: 'History + city', maldives: 'Beach + island',
  'machu-picchu': 'Mountain + nature', zanzibar: 'Beach + island', vilnius: 'Historic city', amsterdam: 'City + nature',
  reykjavik: 'Mountain + nature', marrakech: 'History + city', singapore: 'Big city', lisbon: 'Coastal city',
  'buenos-aires': 'Art + city', 'quebec-city': 'Historic city', bali: 'Beach + island', auckland: 'Harbor + beach',
  'rio-de-janeiro': 'Harbor + beach',
  accra: 'Coastal city', mumbai: 'Big city', copenhagen: 'Harbor + beach', salvador: 'Coastal city', 'new-orleans': 'Art + city',
};
...
export function LittleJetterApp() {
  ...
  function pinchAngle(touches: React.TouchList) {
    const [a, b] = [touches[0], touches[1]];
