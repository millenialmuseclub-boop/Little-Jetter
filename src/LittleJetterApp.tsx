...
function ClassicDoll({ picks, character }: { picks: Picks; character: { style: string; skin: string; hair: string; eyes: string } }) {
  ...
  return <svg className="little-aligned-doll" viewBox="0 0 600 900" role="img" aria-label={`Doll wearing ${wardrobe.tops.find(item=>item.id===picks.tops)?.name}, ${wardrobe.bottoms.find(item=>item.id===picks.bottoms)?.name}, and ${wardrobe.shoes.find(item=>item.id===picks.shoes)?.name}`}>
